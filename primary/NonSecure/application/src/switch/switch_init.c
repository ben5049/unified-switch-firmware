...
#if HW_VERSION == 4
    [SWITCH0] = swv4_sja1105_static_config_default
#elif HW_VERSION == 5
    [SWITCH0] = swv5_0_sja1105_static_config_default,
    [SWITCH1] = swv5_1_sja1105_static_config_default
#endif
};

static const uint32_t switch_static_config_sizes[NUM_SWITCHES] = {
#if HW_VERSION == 4
    [SWITCH0] = SWV4_SJA1105_STATIC_CONFIG_DEFAULT_SIZE
#elif HW_VERSION == 5
    [SWITCH0] = SWV5_0_SJA1105_STATIC_CONFIG_DEFAULT_SIZE,
    [SWITCH1] = SWV5_1_SJA1105_STATIC_CONFIG_DEFAULT_SIZE
#endif
};

static const uint16_t port_speeds[NUM_PORTS] = {
    [PORT0] = PORT0_SPEED_MBPS,
    [PORT1] = PORT1_SPEED_MBPS,
    [PORT2] = PORT2_SPEED_MBPS,
    [PORT3] = PORT3_SPEED_MBPS,
#if HW_VERSION == 4
    [PORT_HOST] = PORT4_SPEED_MBPS,
#elif HW_VERSION == 5
    [PORT4]     = PORT4_SPEED_MBPS,
    [PORT5]     = PORT5_SPEED_MBPS,
    [PORT6]     = PORT6_SPEED_MBPS,
    [PORT_HOST] = PORT7_SPEED_MBPS,
#endif
};
...
sja1105_status_t switch_init() {
    ...
    switch_configs[SWITCH0].variant      = VARIANT_SJA1105Q;
    switch_configs[SWITCH0].timeout      = SWITCH_TIMEOUT_MS;
    switch_configs[SWITCH0].mgmt_timeout = SWITCH_MANAGMENT_ROUTE_TIMEOUT_MS;
    switch_configs[SWITCH0].host_port    = SW0_PORT_HOST;  /* STM32 Host port */
#if HW_VERSION == 4
    switch_configs[SWITCH0].casc_port = SJA1105_NUM_PORTS; /* No cascaded port */
#elif HW_VERSION == 5
    switch_configs[SWITCH0].casc_port = SW0_PORT_SW1; /* Port to switch 1 downstream */
#endif
    switch_configs[SWITCH0].skew_clocks  = true; /* Improves EMI performance */
    switch_configs[SWITCH0].switch_id    = SWITCH0;
    switch_configs[SWITCH0].destmeta_msw = MAC_ADDR_MSW(MAC_ADDR_OCTET1, MAC_ADDR_OCTET2);
    switch_configs[SWITCH0].destmeta_lsw = MAC_ADDR_LSW(MAC_ADDR_OCTET3, MAC_ADDR_OCTET4, MAC_ADDR_OCTET5, MAC_ADDR_OCTET6);

    /* Set the general switch 1 parameters */
#if HW_VERSION == 5
    switch_configs[SWITCH1].variant      = VARIANT_SJA1105Q;
    switch_configs[SWITCH1].timeout      = SWITCH_TIMEOUT_MS;
    switch_configs[SWITCH1].mgmt_timeout = SWITCH_MANAGMENT_ROUTE_TIMEOUT_MS;
    switch_configs[SWITCH1].host_port    = SW1_PORT_SW0;      /* Port to switch 0 upstream */
    switch_configs[SWITCH1].casc_port    = SJA1105_NUM_PORTS; /* No downstream switches */
    switch_configs[SWITCH1].skew_clocks  = true;              /* Improves EMI performance */
    switch_configs[SWITCH1].switch_id    = SWITCH1;
    switch_configs[SWITCH1].destmeta_msw = MAC_ADDR_MSW(MAC_ADDR_OCTET1, MAC_ADDR_OCTET2);
    switch_configs[SWITCH1].destmeta_lsw = MAC_ADDR_LSW(MAC_ADDR_OCTET3, MAC_ADDR_OCTET4, MAC_ADDR_OCTET5, MAC_ADDR_OCTET6);
#endif
    ...
    port_config.connected_switch_handle   = &switch_handles[SWITCH1];
#endif
    status = SJA1105_PortConfigure(&switch_configs[SWITCH0], &port_config);
    if (status != SJA1105_OK) return status;
    ...
    port_config.voltage       = SJA1105_IO_1V8;
    port_config.rgmii_id_mode = SJA1105_RGMII_ID_NONE;
    status                    = SJA1105_PortConfigure(&switch_configs[SWITCH1], &port_config);
    if (status != SJA1105_OK) return status;
    ...
    port_config.connected_switch_handle   = &switch_handles[SWITCH0];
    status                                = SJA1105_PortConfigure(&switch_configs[SWITCH1], &port_config);
    if (status != SJA1105_OK) return status;
    ...
    for (port_index_t i = SWITCH0; i < NUM_PHYS; i++) {
